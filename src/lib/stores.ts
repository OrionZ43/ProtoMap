...
onAuthStateChanged(auth, async (userAuth: User | null) => {
	...
	try {
		// Синхронизация сессии для SSR (выполняется ОДИН раз при входе)
		// Но только если мы не на странице логина/регистрации (там своя логика с 2FA)
		if (browser && !window.location.pathname.match(/^\/(login|register)$/)) {
			...
			async (docSnap) => {
				if (docSnap.exists()) {
					...
					const userProfile: UserProfile = {
						...
						last_daily_bonus:
							data.last_daily_bonus && typeof data.last_daily_bonus.toDate === 'function'
