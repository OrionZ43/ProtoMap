<script lang="ts">
    ...
    // Дата запуска реферальной программы — пользователи ДО этой даты не видят Welcome Bonus
    const PROGRAM_LAUNCH = new Date('2026-03-14T00:00:00.000Z');

    let user: any           = null;
    let loading             = true;
    let isNewUser           = false;   // зарегистрирован после PROGRAM_LAUNCH
    ...
    let conditions          = { email: false, telegram: false };
    ...
    onMount(async () => {
        ...
        onAuthStateChanged(auth, async (u) => {
            user = u;
            if (u) {
                // Читаем createdAt из Firestore чтобы определить новый ли пользователь
                try {
    ...
    async function loadStatus() {
        try {
            ...
            campaignStatus      = d.campaignStatus     ?? 'active';
            bonusAlreadyClaimed = d.bonusAlreadyClaimed ?? false;
            conditions          = d.conditions         ?? { email: false, telegram: false };
            startTimer();
        } catch (e) { console.error(e); }
    }
    ...
    // Показываем блок активации кода новичку только если:
    // — зарегистрирован после запуска программы И ещё не получил бонус
    $: showWelcomeBlock = isNewUser && !bonusAlreadyClaimed;
</script>

<svelte:head>
    <title>// REFERRAL PROTOCOL | ProtoMap</title>
</svelte:head>
...
<div class="page-wrap" style="opacity: {$opacity}">
    ...
    <div class="content-wrap">
        ...
        <header class="page-header" in:fade={{ delay: 80, duration: 600 }}>
            <div class="header-tag">// EVENT.EXE</div>
            <h1 class="page-title glitch" data-text="REFERRAL PROTOCOL">REFERRAL PROTOCOL</h1>
            <p class="page-sub">Invite the pack. Dominate the leaderboard. Claim the throne.</p>
            {#if timeLeft}
                <div class="timer-wrap" in:fade={{ delay: 300 }}>
                    <span class="timer-label">CAMPAIGN ENDS IN</span>
        ...
        <section class="rewards-row" in:fade={{ delay: 180, duration: 500 }}>
            {#if showWelcomeBlock || (user && isNewUser)}
                <div class="reward-card rc-new">
                    ...
                    <div class="rc-amount">500 PC</div>
                    <div class="rc-label">тебе при активации кода</div>
                </div>
                <div class="reward-sep">+</div>
            {/if}
            <div class="reward-card rc-invite">
                <div class="rc-icon">💎</div>
                <div class="rc-amount">250 PC</div>
                <div class="rc-label">рефереру за каждого</div>
            </div>
            <div class="reward-sep">=</div>
            <div class="reward-card rc-winner">
                <div class="rc-icon">👑</div>
                <div class="rc-amount">10 000 PC</div>
                <div class="rc-label">победителю месяца</div>
            </div>
        </section>
            ...
            <div class="left-col">
                ...
                {:else if !user}
                    <div class="panel" in:fade={{ delay: 280 }}>
                        <div class="panel-header"><span class="panel-tag">// ACCESS DENIED</span></div>
                        <p class="muted-text">Войди в систему чтобы участвовать.</p>
                        <a href="/login" class="btn-primary">ВОЙТИ</a>
                    </div>

                {:else}
                    <!-- ── Welcome Bonus — только для новых пользователей ── -->
                    {#if showWelcomeBlock}
                        <div class="panel bonus-panel" in:fade={{ delay: 260, duration: 500 }}>
                            <div class="panel-header">
                                <span class="panel-tag">// WELCOME BONUS</span>
                                <span class="badge-pc">+500 PC</span>
                            </div>
                            ...
                            <p class="bonus-desc">
                                Введи реферальный код того, кто тебя пригласил — и получишь <strong>500 PC</strong> приветственного бонуса. Рефер тоже получит <strong>250 PC</strong>.
                            </p>
                            ...
                            <ul class="checklist">
                                <li class="check-item" class:check-done={conditions.email}>
                                    ...
                                    <div class="check-content">
                                        <span class="check-title">Подтвердить email</span>
                                        {#if !conditions.email}
                                            <span class="check-hint">Проверьте почту и перейдите по ссылке</span>
                                        {/if}
                                    </div>
                                </li>
                                <li class="check-item" class:check-done={conditions.telegram}>
                                    ...
                                    <div class="check-content">
                                        <span class="check-title">Привязать Telegram</span>
                                        {#if !conditions.telegram}
                                            <span class="check-hint">Профиль → Безопасность → Привязать Telegram</span>
                                        {/if}
                                    </div>
                                </li>
                                <li class="check-item">
                                    ...
                                    <div class="check-content">
                                        <span class="check-title">Вступить в <a href="https://t.me/proto_map" target="_blank" rel="noopener" class="tg-link">@proto_map</a></span>
                                        <span class="check-hint">Проверяется в момент активации кода</span>
                                    </div>
                                </li>
                            </ul>
                            ...
                            {#if claimResult}
                                <div class="result-msg"
                                    ...
                                    {#if claimResult.success}
                                        ✅ Код активирован! Ты получил <strong>+{claimResult.newUserEarned} PC</strong>. Рефер получил +{claimResult.referrerEarned} PC.
                                    {:else}
                                        ⚠️ {claimResult.error}
                                    {/if}
                                </div>
                    ...
                    {#if isNewUser && bonusAlreadyClaimed}
                        <div class="panel panel-success" in:fade={{ delay: 260 }}>
                            <div class="panel-header">
                                <span class="panel-tag">// WELCOME BONUS</span>
                                <span class="badge-done">✓ ПОЛУЧЕН</span>
                            </div>
                            <p class="muted-text">Приветственный бонус уже зачислен на твой счёт. 🎉</p>
                        </div>
                    {/if}
                    ...
                    <div class="panel" in:fade={{ delay: 320, duration: 500 }}>
                        ...
                        {#if myCode}
                            ...
                            <p class="share-hint">
                                За каждого кто активирует твой код ты получишь <strong>250 PC</strong>. Тот у кого больше рефералов за месяц забирает ещё <strong>10 000 PC</strong>.
                            </p>
                        {:else}
                            <p class="muted-text">У тебя ещё нет реферального кода.</p>
                            <button class="btn-primary" on:click={handleGetCode} disabled={codeLoading}>
                                {codeLoading ? 'ГЕНЕРАЦИЯ...' : '⚡ ПОЛУЧИТЬ КОД'}
                            </button>
                        {/if}
                    </div>
                ...
                <div class="panel" in:fade={{ delay: 440, duration: 500 }}>
                    <div class="panel-header"><span class="panel-tag">// ПРАВИЛА</span></div>
                    <p class="muted-text" style="margin-bottom:1.25rem">Реферал засчитывается только если новый пользователь выполнил все условия:</p>
                    <ol class="rules-list">
                        <li>
                            <span class="rule-n">01</span>
                            <div><strong>Email подтверждён</strong><span>Без верификации код не активируется</span></div>
                        </li>
                        <li>
                            <span class="rule-n">02</span>
                            <div><strong>Telegram привязан</strong><span>Через раздел «Безопасность» в профиле</span></div>
                        </li>
                        <li>
                            <span class="rule-n">03</span>
                            <div><strong>Вступление в <a href="https://t.me/proto_map" target="_blank" rel="noopener" class="tg-link">@proto_map</a></strong><span>Проверяется на сервере в момент активации</span></div>
                        </li>
                    </ol>
                    <p class="rules-note">⚠️ Один аккаунт — один код. Нельзя активировать свой код. Программа для новых пользователей, зарегистрированных после 14 марта 2026.</p>
                </div>
            </div>
            ...
            <div class="right-col">
                ...
                <div class="panel" in:fade={{ delay: 300, duration: 500 }}>
                    ...
                    {#if leaderboard.length === 0}
                        <div class="lb-empty">
                            <span>📡</span>
                            <span>Нет данных. Стань первым!</span>
                        </div>
                    {:else}
                        <ol class="lb-list">
                            {#each leaderboard as entry, i}
                                <li class="lb-row"
                                    ...
                                    <a href="/profile/{entry.username}" class="lb-name">
                                        {entry.username}
                                        {#if entry.uid === user?.uid}<span class="you-tag">YOU</span>{/if}
                                    </a>
                    ...
                    <div class="lb-footer">
                        <span>Победитель получит ещё</span>
                        <span class="lb-prize font-display">10 000 PC 👑</span>
                    </div>
                </div>

                <!-- Инструкция -->
                <div class="panel" in:fade={{ delay: 420, duration: 500 }}>
                    <div class="panel-header"><span class="panel-tag">// КАК ПОБЕДИТЬ</span></div>
                    <div class="howto">
                        <div class="ht-step">
                            <span class="ht-n">01</span>
                            <span>Получи реферальный код выше</span>
                        </div>
                        <div class="ht-arrow">↓</div>
                        <div class="ht-step">
                            <span class="ht-n">02</span>
                            <span>Поделись с друзьями-протогенами, синтами, примагенами</span>
                        </div>
                        <div class="ht-arrow">↓</div>
                        <div class="ht-step">
                            <span class="ht-n">03</span>
                            <span>Друг регистрируется → верифицирует email → привязывает Telegram → вступает в <a href="https://t.me/proto_map" target="_blank" rel="noopener" class="tg-link">@proto_map</a> → вводит твой код</span>
                        </div>
                        <div class="ht-arrow">↓</div>
                        <div class="ht-step ht-step--win">
                            <span class="ht-n">04</span>
                            <span>Ты: <strong>+250 PC</strong> мгновенно. Друг: <strong>+500 PC</strong>. Лидер месяца получает ещё <strong>+10 000 PC</strong>.</span>
                        </div>
                    </div>
                </div>
...
<style>
    ...
    .page-title {
        font-family:'Chakra Petch',monospace;font-size:clamp(2rem,6vw,3.8rem);
        font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:.08em;
        line-height:1;margin-bottom:.6rem;position:relative;display:inline-block;
    }
    ...
    .rc-new    { border-color:rgba(0,240,255,.2); }
    .rc-invite { border-color:rgba(0,240,255,.15); }
    .rc-winner { border-color:rgba(252,238,10,.3);box-shadow:0 0 18px rgba(252,238,10,.07); }
    .rc-new:hover    { border-color:rgba(0,240,255,.4);box-shadow:0 0 18px rgba(0,240,255,.1); }
    .rc-invite:hover { border-color:rgba(0,240,255,.35);box-shadow:0 0 15px rgba(0,240,255,.08); }
    .rc-winner:hover { border-color:rgba(252,238,10,.55);box-shadow:0 0 25px rgba(252,238,10,.14); }
    .rc-icon   { font-size:1.6rem;margin-bottom:.2rem; }
    .rc-amount { font-family:'Chakra Petch',monospace;font-size:1.6rem;font-weight:900; }
    ...
    .bonus-bg-text {
        ...
        font-family:'Chakra Petch',monospace;font-size:5rem;font-weight:900;
        color:rgba(0,240,255,.04);pointer-events:none;user-select:none;white-space:nowrap;
        letter-spacing:.05em;
    }
    ...
    .bonus-desc strong { color:var(--cy); }
    ...
    .check-title { font-size:.84rem;color:#cbd5e1; }
    .check-item.check-done .check-title { color:#4ade80; }
    ...
    .code-input:focus { border-color:var(--cc); }
    ...
    .stat-hi    { color:#e2e8f0; }
    .stat-lo    { color:#475569;font-size:.63rem; }
    .stat-dot   { color:#1e293b; }
    .stat-rank  { color:var(--cy); }
    ...
    .lb-refs { font-size:.8rem;color:#94a3b8; }
    .lb-unit { font-size:.6rem;color:#475569; }
    .lb-pc   { font-size:.75rem;color:rgba(252,238,10,.65); }
    ...
    .tg-link    { color:var(--cc);text-decoration:none;border-bottom:1px dashed rgba(0,240,255,.35); }
