...
<script lang="ts">
	...
	// FIX (баг 3): Импортируем ChannelsFeed — раньше его не было вообще!
	...
	// Реактивный запуск inbox — ждём пока userStore загрузится
	...
	onMount(() => {
		...
		const unsub = chat.subscribe((state) => {
			if (state.pendingDM && $userStore.user) {
				...
				openChat({ id: chatId, partner, lastMessage: '', lastMessageTimestamp: null, unread: 0 }, myUid);
			}
		});
	...
	function openFavorites() {
		...
		// FIX (баг 2): Кнопка «Избранное» открывает self-chat напрямую,
		// но self-chat НЕ попадает в $chats (отфильтровано в dmStore),
		// поэтому дубликата в списке больше нет.
		openChat(
			{
				id: `${me.uid}_${me.uid}`,
		...
		// При клике на Избранное — переключаемся на DM вкладку чтобы видеть хедер
		activeTab = 'dm';
	}

	// ── Переключение вкладки Каналы ───────────────────────────────────────
	function switchToChannels() {
		activeTab = 'channels';
		// Закрываем активный DM-чат чтобы не путаться
		...
		try {
			...
			await new Promise<void>((res, rej) => {
				task.on(
					'state_changed',
					(s) => {
						uploadProgress = Math.round((s.bytesTransferred / s.totalBytes) * 100);
					},
					rej,
					async () => {
			...
			mediaRecorder.onstop = async () => {
				...
				try {
					...
					const task = uploadBytesResumable(
						storageRef(
							storage,
							`chat_media/${$activeChat.id}/voice/${msgRef.id}.webm`
						),
						blob,
	...
	async function _write(fields: Partial<DMMessage> & { type: string }, ref?: ReturnType<typeof doc>) {
		if (!$activeChat || !$userStore.user) return;
		...
		// setDoc + merge — безопасно создаёт документ чата если его нет
		await setDoc(
			doc(db, 'chats', chatId),
			{
				lastMessage: preview,
...
{#if activeTab === 'channels'}
	<div class="page channels-mode" in:fade={{ duration: 200 }}>
		<aside class="sidebar">
			<div class="sidebar-top">
				<span class="sidebar-title">// СООБЩЕНИЯ</span>
			</div>
			<nav class="tabs">
				<button class="tab" on:click={() => { activeTab = 'dm'; goto('/messages', { replaceState: true }); }}>
			...
			<!-- В режиме каналов sidebar пустой — контент в main -->
			<div class="sidebar-hint">
				<span>Выбери канал справа</span>
			</div>
		</aside>

		<main class="main-area">
			<!-- ChannelsFeed занимает всю правую колонку -->
			<ChannelsFeed />
		</main>
	</div>
	...
	<div class="page" in:fade={{ duration: 200 }}>
		...
		<aside class="sidebar">
			...
			<nav class="tabs">
				...
				<button class="tab" on:click={switchToChannels}>
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M22 8.35V20a2 2 0 01-2 2H4a2 2 0 01-2-2V8.35A2 2 0 012.61 7l8-5a2 2 0 012.78 0l8 5A2 2 0 0122 8.35z"/><path d="M15 22v-4a5 5 0 00-6 0v4"/></svg>
					Каналы
				</button>
				<button class="tab fav-tab" on:click={openFavorites}>
			...
			<div class="chat-list">
				<!-- FIX (баг 2): Хардкодная кнопка Избранного — ОДНА.
				     Self-chat из Firestore теперь отфильтрован в dmStore,
				     поэтому дубликата из $chats больше нет. -->
				{#if $userStore.user}
					<button
				...
				{#each $chats as dmChat (dmChat.id)}
					...
					>
						<div class="ava-wrap">
							<img
								src={dmChat.partner.avatarUrl ||
									`https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${dmChat.partner.username}`}
				...
				{#if $chats.length === 0 && !$userStore.loading && $userStore.user}
		...
		<main class="main-area">
			...
			{:else}
					...
					<div class="chat-header-info">
						{#if isFav}
							<div class="fav-icon-sm">
								<svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M17 3H7c-1.1 0-2 .9-2 2v16l7-3 7 3V5c0-1.1-.9-2-2-2zm0 15l-5-2.18L7 18V5h10v13z"/></svg>
							</div>
							<span class="partner-name">Избранное</span>
						{:else}
							<img
								src={$activeChat.partner.avatarUrl ||
									`https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${$activeChat.partner.username}`}
								alt=""
								class="partner-ava"
							/>
				...
				<div
					...
					on:click={() => (reactionPanelMsg = null)}
				>
							...
							<div class="bubble-wrap" class:own={isOwn}>
								...
								>
									...
									{:else if (msg.type === 'sticker' || msg.type === 'STICKER') && msg.sticker_pack_id && msg.sticker_id}
										<img
											src={getSticker(msg.sticker_pack_id, msg.sticker_id)}
											alt="sticker"
											class="sticker-img"
											loading="lazy"
										/>
									...
									{#if msg.type !== 'sticker' && msg.type !== 'STICKER'}
										<span class="msg-time">
											...
											{#if isOwn}
												{@const read =
													$activeChat && msg.read_by?.[$activeChat.partner.uid]}
						...
						<div class="sticker-grid">
							{#if activePack}
								{#each activePack.stickers as filename (filename)}
									<button
										class="sbtn"
										on:click={() => sendSticker(activePack!.id, filename)}
									>
										<img
											src={getSticker(activePack!.id, filename)}
											alt=""
											loading="lazy"
											class="simg"
										/>
				...
				<div class="input-area">
					...
					<div class="input-row">
						...
						<button
							class="tool"
							on:click={() => (showStickerPicker = !showStickerPicker)}
							title="Стикеры"
						>
						...
						{#if isRecording}
							<div class="rec-wrap">
								...
								<button class="stop-btn" on:click={toggleRecording}>
...
<style>
	...
	/* Каналы занимают всю ширину страницы — sidebar тонкий, main широкий */
	.channels-mode .main-area {
	...
	.bubble {
		padding: 0.55rem 0.7rem 1.45rem;
		border-radius: 14px;
		position: relative;
	...
	.bubble-wrap:hover .react-btn {
		animation: delayed-show 0.8s forwards;
	}
