...
## 2026-09-11 (6) — Отзыв согласия на шагомер, рейтинг по согласию, документы 5.2

**Что.**

- **Отзыв удаляет данные.** `revokeConsent("activity_data")` удаляет с сервера
  данные о шагах (`functions/src/stepperData.ts`: `stepper/{uid}` вместе с
  `claim_history`, запись в рейтинге, ключи идемпотентности) и заодно отзывает
  показ в рейтинге. Раньше отзыв ставил только пометку в журнале, а Политика
  обработки обещает удаление в 15 дней. Начисленные ProtoCoins остаются.
- **Повторная оплата тех же шагов.** Сколько шагов за день уже оплачено, сервер
  помнил только в удаляемых счётчиках: отозвал согласие, дал снова — и те же
  шаги оплачиваются второй раз. Если сегодня было начисление, до конца суток
  остаётся отметка `claimsLockedUntil`, в которой нет ни одного числа шагов.
- **`stepperClaim`** не начисляет при `activity_data_consent == false`. Если поля
  нет, начисление пока проходит: приложение ещё не спрашивает согласие.
- **Рейтинг — по отдельному согласию** `activity_leaderboard` (решение Ориона).
  Рейтинг читает любой вошедший пользователь, а в документах о нём не было ни
  слова — то есть специальные данные показывались другим людям без согласия. В
  рейтинг теперь уходят только суммы; шаги по дням переехали в
  `stepper/{uid}.stepsByDay`.
- **`hourStats`** в журнале начислений больше не сохраняется: Политика обещала
  итоги дня, а хранились шаги по часам.
- **`claimedDate`.** Сервер пишет поле, которое читает `getClaimedSteps` в
  приложении. Без него оплаченные шаги в приложении всегда были нулём.
- **Чистка по срокам** падала на двух целях из пяти при каждом запуске:
  - `claim_history` — не было индекса группы коллекций; добавлен в
    `firestore.indexes.json`;
  - `2fa_codes` — у поля с TTL нет одиночного индекса, запрос по сроку
    невозможен. Цель убрана, коды удаляет TTL.
- **`accountPurge`** удаляет и ключи идемпотентности шагомера.
- **Документы.** Политика конфиденциальности 5.2 и Политика обработки 1.1:
  рейтинг по согласию (абзац в п. 2.5 и строка в приложении 1), удаление данных
  о шагах при отзыве, разбивка по часам. Соглашение не менялось, осталось 5.1.
  Залиты.

**Данные на проде** — разовая миграция:
- снят ложный `activity_data_consent: false` у 2 пользователей: это след бага
  из записи (5), записи об отзыве у них нет;
- `hourStats` вычищен из 164 записей;
- 3 записи рейтинга без согласия удалены, их итоги перенесены в `stepper/{uid}`.

**Проверено.** Боевой прогон на временной учётке, 19 из 19:
- начисление проходит; `claimedDate`, шаги по дням и итог лежат в
  `stepper/{uid}`; журнал без разбивки по часам;
- без согласия записи в рейтинге нет, с согласием — только суммы, без `history`;
- отзыв рейтинга удаляет запись и не трогает шагомер;
- отзыв шагомера отзывает оба согласия, оставляет только `claimsLockedUntil`,
  удаляет журнал, ключи и рейтинг, а ProtoCoins остаются (100 → 140);
- после отзыва начисления нет; рейтинг без шагомера не даётся; после
  повторного согласия те же шаги до конца суток не оплачиваются.

Повторный прогон миграции — 0 / 0 / 0. Запрос чистки `claim_history` по сроку на
проде работает. Документы прошли проверку структуры скриптом заливки.

**За Денисом** — пункт 5 памятки: экран согласия с двумя галочками,
переключатель рейтинга, отзыв при выключении шагомера и при отзыве разрешения в
настройках Android.

---

## 2026-09-11 (5) — Памятка для Дениса и правка флага шагомера

**Что.**

- `recordConsents`: флаг `activity_data_consent` теперь только включается,
  выключает его только `revokeConsent`. Раньше функция писала флаг по тому, есть
  ли `activity_data` в текущем вызове, и повторное принятие документов (экран
  согласий на сайте шлёт только обязательные) молча ставило `false` тем, кто
  шагомер включал. Там же: согласие на шагомер можно дать отдельным вызовом, если
  обязательные приняты на текущую редакцию, — раньше такой вызов отклонялся как
  «не отмечены обязательные».
- `docs/ANDROID_HANDOFF.md` и страница-артефакт с кнопкой «Скопировать для
  Claude»: что сделать в приложении до и после 18.09, вопросы к Денису,
  контракты функций, тексты для `strings.xml` (выгружены из локалей сайта с
  экранированием для Android), формат `<table>`. Пункты пронумерованы сквозь,
  чтобы на них можно было ссылаться в ответе.

**Что нашлось при сверке с кодом приложения** (копия от 15.07.2026, других у нас нет):

- Приложение зовёт функции в `us-central1`: `AppModule` создаёт экземпляр без
  региона, `AuthViewModel`, `CreateUsernameScreen` и `SecurityViewModel` задают
  `us-central1` явно. Там с 03.09 остался только `onUserCreated`. Если релиз
  такой же, эти вызовы получают `NOT_FOUND`. Вынесено первым пунктом памятки.
- `sendImageMessage` и `sendVoiceMessage` приложение вызывает, а на бэкенде их
  нет ни в исходниках, ни на проде. Остальные вызовы приложения сверены с
  `firebase functions:list` — все есть в `europe-west1`.
- `recordConsents` приложение не вызывает вообще: согласие — одна галочка в
  интро, версии в SharedPreferences, возраст не спрашивается. Парсер документов
  молча пропускает `<table>`, так что приложения к Политике обработки в
  приложении не видны.

**Проверено.** Боевой прогон на временной учётке с токенами Auth и App Check,
11 из 11: одно `activity_data` до принятия документов — отказ
`FAILED_PRECONDITION`; четыре обязательных — зеркала 5.1 / 5.1, флага нет;
`activity_data` отдельно — принято, флаг `true`, обязательные не продублированы;
повторное принятие четырёх — флаг остался `true`; `revokeConsent` — `false`;
обязательное отдельно не отзывается; `getMyConsents` отдаёт 9 записей. Учётка
и её записи в журнале удалены.
