...
## 2026-09-11 (4) — Из документов убрано то, чего в Сервисе нет

**Что.** По замечанию Ориона убраны утверждения о несуществующем:

- п. 1.3 Политики обработки: «работников нет», «видеонаблюдение не
  осуществляется», «аналитические и рекламные cookie не используются». Всё это
  пришло из примерной формы ЦЗПД, где графы «применяется / не применяется»
  предложены для всех случаев. Осталось одно утверждение: сайт использует только
  технические cookie.
- п. 2.4 Политики обработки — про ФИО, которые не обрабатываются. Он дублировал
  Политику конфиденциальности; пункты 2.5–2.6 стали 2.4–2.5.
- Возраст: «такой возможности Сервис не предоставляет», «отдельного ограничения
  у раздела нет», «сознательно не собираем» заменены на «регистрация доступна с
  16 лет». Поправлено в трёх документах и в подсказке на регистрации и в экране
  согласий.
- «В Сервисе реклама отсутствует» и «Оператором не хранятся» в таблице.

**Что оставлено сознательно** — утверждения, которые работают как гарантия или
раскрытие для человека: не продаём данные; пароли не храним; E2E-шифрования нет,
и сервер читает сообщения; голос не биометрия; ProtoCoins не деньги; трек
перемещений не храним; ФИО при даче согласия не запрашиваются. Последнее —
ссылка на часть вторую пункта 6 статьи 5 Закона: она объясняет, почему в
согласии нет ФИО.

**Версии не менялись.** Записей в журнале согласий на 5.1 ещё нет (проверено:
0), документы вступают в силу 18.09.

---

## 2026-09-11 (3) — Выкат документов, гейта и очистки аккаунта

**Дата вступления в силу — 18.09.2026.** В документах стояло 14.09, но выкат
пришёлся на 11.09, и три дня до вступления в силу — это не срок на ознакомление.
Поставили день выката плюс неделю; экран согласий включается в ту же дату.

**Что выкачено:**

- функции `deleteAccount`, `enforceRetention`, `telegramWebhook`;
- индексы Firestore;
- документы в `system/licenses`: Политика 5.1, Соглашение 5.1, Политика
  обработки 1.0;
- фронтенд:
  - гейт и баннер;
  - `/personal-data-policy` и общий `LegalPage`;
  - редизайн входа и регистрации;
  - `?focus=` на карте и кнопка «Открыть на карте» в профиле;
- одноразово вычищены персональные поля из старых `whining_attempts`.

**Найдено при выкате.** `firestore.indexes.json` отставал от прода на 5
составных индексов и 2 TTL-политики. Без `--force` CLI их не трогает. С
`--force` снёс бы — и сломались бы лента лички, новости и рейтинг рефералов, а
TTL сообщений и краш-игр перестал бы работать. Файл заменён выгрузкой
`firebase firestore:indexes`.

**Как проверялось.** Боевой прогон с настоящими токенами Auth и App Check;
токен App Check выпущен через Admin SDK. Тестовая учётка загрузила аватар через
`uploadAvatar` и удалилась через `deleteAccount`: учётка, профиль и файл в
Cloudinary удалены — 8 проверок из 8. Первый прогон упал ещё на входе: App
Check принуждается и для самого Firebase Auth, поэтому токен нужен уже запросу
`signInWithCustomToken`.

---

## 2026-09-11 (2) — Полная очистка аккаунта и сроки хранения

**Задача.** Орион принял решения по открытым пунктам сверки:

- расшифровка голосовых остаётся только в личном чате;
- отдельной политики cookie не будет;
- метки спящих аккаунтов остаются на карте;
- сроки хранения из приложения 1 — «если убрать нельзя, надо делать»;
- у удалённого аккаунта тексты сообщений обезличиваются, медиа удаляются.

**Сроки убрать нельзя.** Это обязательная графа: пункт 8 статьи 4 Закона и
форма ЦЗПД. Срок согласия уже записан в тексте, который люди принимают.
Заявления и переписка лежат в почте — их чистят вручную. В коде нужно удаление
после трёх лет без входа, и эта задача упёрлась в `deleteAccount`.

**`deleteAccount` делал меньше, чем обещала Политика.** Инвентаризация прода
(`listCollections` и имена полей, без содержимого) показала, что после удаления
оставалось:

- аватар в Cloudinary — хотя Политика прямо обещает его удалить;
- подколлекция `private` с FCM-токенами и комментарии на странице профиля;
- личка, посты и владение каналами, подписки;
- жалобы и флаги модерации;
- шагомер с историей и рефералка;
- присутствие в RTDB и бета-телеметрия Android.

**Что сделано:**

- `accountPurge.ts` — единая очистка с режимом «только посчитать»:
  - тексты обезличиваются, медиа удаляются;
  - пересланное медиа не трогается: файл принадлежит тому, кто отправил его
    первым;
  - удаляются только файлы из папок пользовательских загрузок, стикеры и
    новости не затрагиваются никогда;
  - учётная запись Auth удаляется последней, поэтому недоделанную очистку можно
    повторить.

  Личку ищем по `participantIds`, каналы перебираем. Так новые индексы
  коллекционных групп для `messages` и `posts` не понадобились.
- `deleteAccount` теперь вызывает `purgeAccount`. Сигнатура и ответ прежние,
  Android-клиент это не затрагивает.
- `enforceRetention` удаляет учётные записи после трёх лет без входа. «Вход» —
  самое позднее из времени входа и обновления токена в Firebase Auth. Пока не
  выставлен `RETENTION_APPLY`, работает вхолостую; удаляет не больше 25 за
  запуск; администраторов не трогает.
- `mapCache.ts` — `clearMapCache` вынесен из `index.ts`, иначе был бы
  циклический импорт.
- Попутно исправлено:
  - журнал согласий чистился от даты выдачи, теперь — от `revokedAt`;
  - повторная очистка перезаписывала `accountDeletedAt`;
  - ключи Cloudinary не были объявлены у `deleteAccount` и `enforceRetention`,
    и аватар молча оставался бы;
  - `whining_attempts` писал персональные данные, которые его статистике не
    нужны;
  - индекс `comments.author_uid` существовал только в консоли.
- Документы: Telegram-часть Сервиса сужена до чата и канала сообщества ProtoMap;
  сроки хранения текстов и медиа разведены.

**Как проверялось.** Тест `functions/_purge_test.cjs` на проде, удалён после прогона:

- две тестовые учётки с данными во всех местах, где они живут; даты записей —
  2020 год, чтобы не всплыть в живых лентах;
- сухой прогон ничего не меняет;
- боевой проходит все проверки;
- повторный запуск не падает и не сдвигает дату удаления в журнале согласий;
- всё созданное убирается, в том числе если триггер `onUserCreated`
  сработает с опозданием.

Заодно посчитано, сколько аккаунтов не заходили больше трёх лет: 0 из 230.

**Не проверено тестом:** удаление аватара в Cloudinary — ключей локально нет.
Проверить после деплоя.

---

## 2026-09-11 — Возраст 16 и Политика в отношении обработки персональных данных

**Задача.** Два замечания преподавателя Ориона: проверить возрастной ценз (17 или
18) и сделать отдельную страницу политики обработки персональных данных как
требование законодательства Беларуси.

**Возраст.** По закону — ни 17, ни 18, а 16: пункт 9 статьи 5 Закона № 99-З
(до шестнадцати лет согласие даёт законный представитель). Цифра 17 была
рейтингом ESRB Mature 17+ из Google Play — категорией содержания, которая
слиплась с возрастом дачи согласия. Требование закона в документах записано не
было вовсе. Орион выбрал минимум 16 и отказ в регистрации младше 16.

**Отдельная Политика.** Абзац третий пункта 3 статьи 17; ЦЗПД прямо относит к
обязанным операторам физических лиц. Собрана по примерной форме НЦЗПД — её
текст лежит прямо на cpd.by, а ссылки на `.docx` подгружаются скриптом и в
разметке их нет.

**Что сделано:**

- `privacy_policy_v5.xml`, `terms_of_service_v5.xml` → 5.1. В них:
  - ценз 16;
  - фактические регионы хранения;
  - фактическая аудитория карты;
  - срок хранения учётной записи, приведённый к сроку согласия;
  - отсылка к отдельной Политике.
- `personal_data_policy_v1.xml` — новый документ. Собран генератором из
  выверенного текста 5.1. Новых правовых оснований сверх него добавлены только
  стандартные строки примерной формы: заявления по ст. 14, переписка, запросы
  госорганов.
- Узел `<table>` для приложений:
  - `legalLoader.ts`, тип `TableNode`;
  - `LegalDocRenderer.svelte` — на узком экране строки становятся карточками с
    подписями из `data-label`;
  - валидатор в `upload-legal-docs.mjs`.

  Версии в скрипте теперь у каждого документа свои.
- `LegalPage.svelte` — общая оболочка трёх юридических страниц. Копии разметки
  успели разойтись:
  - вкладка Соглашения показывала сырой ключ `usage_terms`;
  - в пустом состоянии были эмодзи;
  - навигация была на русском в обход i18n;
  - у Соглашения была голубая сетка.
- Голубые акценты документа — подзаголовки, карточки, иконка контакта —
  перекрашены в жёлтый. Голубой оставлен только у info-алертов, где это
  смысловой цвет.
- Регистрация и экран согласий: пояснение под галочкой возраста и ссылка на
  Политику. Ссылка добавлена и в подвал. `/personal-data-policy` выведена
  из-под гейта.

**Найдено по ходу — документы 5.0 расходились с фактами:**

- Storage находится в `us-central1` (США), а в 5.0 было «Варшава».
- Cloud Functions и RTDB находятся в `europe-west1` (Бельгия).
- Карта публична, а в 5.0 было «доступна зарегистрированным».

Всё это исправлено в 5.1. Публичность карты сначала проверена анонимным
вызовом `getLocations` (ответ 200, 153 метки) и только потом вписана в
документы.

**Как проверялось:**

- скрипт заливки в сухом прогоне — все три документа проходят валидацию;
- `svelte-check` — 30 ошибок и 193 предупреждения, столько же, сколько до правок;
- Playwright:
  - страница на RU и EN, таблицы на 13 и 7 строк;
  - на ширине 390 px нет горизонтальной прокрутки;
  - заголовки вкладок;
  - шаг согласий на регистрации.

**Не сделано, требует решения** — полный список в `docs/COMPLIANCE_99Z.md`,
раздел 2026-09-11:

- автоматическая расшифровка голосовых в Telegram-боте через Gemini (США) без
  согласий и без упоминания в документах;
- отдельная политика cookie;
- спящие учётные записи без согласия;
- сроки хранения, которые не исполняет код;
- рейтинг в Google Play;
- Android: ценз 16 и поддержка `<table>`, если приложение будет показывать новую
  Политику.

---

## 2026-09-07 — Экран согласий для существующих пользователей

**Задача.** Документы вышли в редакции 5.0 (вступает в силу 14.09.2026), а
галочки собирались только на регистрации. Из 229 учётных записей запись в
журнале согласий была ровно у одной — и у остальных не было способа её
поставить.

**Почему не разлогин.** Форма входа согласий не собирает: `recordConsents`
вызывался только из `register/+page.svelte`. Массовый разлогин вернул бы людей
в те же аккаунты с тем же нулём в журнале — раздражение без юридического
результата.

**Что сделано:**

- `functions/src/consents.ts` — `recordConsents` дополнительно пишет в документ
  пользователя зеркала `consents_privacy_version` / `consents_tos_version` /
  `consents_updated_at`. Источник истины остаётся коллекцией `consents`; зеркала
  нужны, чтобы не ходить в журнал на каждую отрисовку страницы. Подделать их
  клиент не может: правило на `users` разрешает владельцу менять только
  `fcmToken` и `lastBackupSignature`. **Задеплоено.**
- `hooks.server.ts` + `app.d.ts` — зеркала едут в `locals.user`. Документ
  пользователя и так читается на каждый запрос, так что проверка бесплатна.
- `+layout.server.ts` — `needsConsent` считается на сервере: клиентскую проверку
  обошёл бы любой, кто откроет консоль. Активируется с `CONSENT_GATE_FROM`
  (14.09.2026, минское время) — раньше даты вступления в силу требовать принятия
  нельзя. Из-под перекрытия выведены сами документы (иначе пришлось бы принимать
  нечитаемое), `/banned`, страницы входа и `/settings/security` — там удаление
  аккаунта, а отказ обязан быть выполнимым.
- `ConsentGate.svelte` — блокирующий экран с четырьмя непредзаполненными
  галочками, переключателем языка (экран перекрывает шапку, а согласие должно
  быть осознанным), выходом и ссылкой на удаление аккаунта.

**Найдено по ходу:**

- **`LegalUpdateBanner` был импортирован в `+layout.svelte`, но не отрисован** —
  то есть уведомления об обновлении документов не видел никто и никогда.
  Добавлен в разметку.
- **Баннер и `CookieBanner` прибиты к одному низу экрана** и накладывались.
  Теперь баннер документов ждёт, пока не решён вопрос с куки.
- Баннер был целиком на русском в обход i18n. Тексты вынесены в `legal.*`
  в трёх локалях (`ru`, `ru_april`, `en`).
- Символы вместо иконок (`⚖`, `→` в баннере; `ℹ`, `⚠`, `✕`, `📡` в
  `LegalDocRenderer`) заменены на SVG.

**Как проверялось.** `scripts/check-consent-gate.mjs` — Playwright на живом
дев-сервере: гейт появляется, кнопка заблокирована до всех галочек, ссылки под
гейтом не кликаются, принятие закрывает экран, перезагрузка его не возвращает.
Плюс сверка в Firestore: журнал пополнился, зеркала выставлены в 5.0.

Первый прогон вскрыл то, чего не видно по коду: запись шла, а гейт оставался —
функция с зеркалами была изменена локально, но не задеплоена.

---

## 2026-09-04 — Промо-ролик: съёмка сайта браузером и монтаж

**Что.** `scripts/showcase/` — два скрипта без внешних сервисов:

- `capture.mjs` — Playwright входит на сайт под тестовым аккаунтом и снимает
  вертикальное видео 540×960 по сценам: заставка → карта целиком → перелёт к метке
  с карточкой → профиль → слоты со спином → магазин → финальная карточка.
  Границы сцен пишутся в `raw/scenes.json`.
- `build.mjs` — ffmpeg режет, поднимает до 1080×1920 (lanczos) и собирает:
  `promo.mp4` (H.264, с подписями — для TikTok и Telegram),
  `showcase.webm` (VP9, без текста — под витрину на странице входа),
  `poster.jpg`.

**Зачем.** Нужен рекламный ролик и живая витрина вместо статичного скриншота.
Ручной монтаж пришлось бы переделывать после каждого изменения интерфейса;
скрипт пересобирает всё одной командой.

**Решённые по ходу проблемы, каждая — отдельный класс ошибок:**

- *Подписи разъезжались с картинкой.* Тайминги были заданы в секундах вручную, а
  сцены при каждой пересъёмке длятся по-разному — «меняй на оформление профиля»
  оказывалось поверх чатов. Теперь подпись привязана к имени сцены и живёт ровно
  столько, сколько идёт сцена.
- *Чат снять нечем.* Личка у аккаунта пустая, в общем чате последние сообщения —
  «кто нибудь в этом чате жив?», в списке каналов превью с анонсом закрытия
  проекта. Бит заменён на страницу профиля.
- *Четыре секунды чёрного экрана перед профилем.* Vite в dev-режиме компилирует
  маршрут при первом заходе. Маршруты прогреваются до старта отсчёта.
- *Профиль открывался «взломом брандмауэра».* `CinematicLoader` занимал всю сцену —
  кликаем `.skip-btn`.
- *Барабаны в слотах не крутились.* Клик через `force: true` проходил до гидратации:
  разметка есть, обработчик ещё не привязан. Ждём гидратацию, кликаем обычным
  кликом, сверяем баланс до и после.
- *В магазине «0 PC».* Баланс подтягивается с задержкой и потом ещё крутится
  анимацией `tweened`. Ждём, пока цифра перестанет меняться, — иначе в кадр
  попадает промежуточное значение.
- *Начало и конец висели на чужих кадрах.* Заставка и финал теперь рисуются
  в браузере (`drawCard`) логотипом и шрифтами сайта.

**Как проверялось.** Покадрово: `ffmpeg -ss <t>` на середину каждой сцены,
просмотр кадра, правка, пересъёмка. Так и вскрылись пустой чат, «0 PC»,
чёрный экран и неподвижные барабаны — по логам всё выглядело успешным.

**Побочно найдено, не чинилось:** 404 на десяток файлов в `static/sounds/`
(`vd_*.mp3`, `entercasino.mp3`) и `static/halloween/bat.svg` — звуки казино
запрашиваются, но их нет в репозитории.

**Не в гите:** `scripts/showcase/.env` (учётка для съёмки), `raw/`, `out/`.
