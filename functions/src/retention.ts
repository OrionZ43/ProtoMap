 ...
 *
 * `2fa_codes` тоже нет: их удаляет TTL-политика Firestore по `expiresAt`
 * (см. firestore.indexes.json). TTL выключает у поля одиночный индекс, так что
 * запрос по сроку к этой коллекции невозможен — цель падала на каждом запуске.
 */
