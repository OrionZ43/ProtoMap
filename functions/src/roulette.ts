...
export type GameEventType = 'item_used' | 'shoot' | 'reload' | 'skip_turn' | 'game_over';
...
const rtdb = () =>
	admin.app().database('https://protomap-1e1db-default-rtdb.europe-west1.firebasedatabase.app');
	...
	if (action === 'item_ad') {
		...
		if (ammo.length === 0 && pub.st === 'a') {
			...
			events.push({
				type: 'reload',
				actor: isPlayer ? 'player' : 'orion',
				shellCount: ammo.length - prevLen,
				log: '🔄 Новый магазин'
			});
		}
		return true;
	...
	if (action === 'shoot_self' || action === 'shoot_enemy') {
		...
		if (pub.php <= 0 || pub.ohp <= 0) {
			events.push({
				type: 'game_over',
				actor: isPlayer ? 'player' : 'orion',
				winner: pub.st === 'p' ? 'player' : 'orion',
				log: pub.st === 'p' ? 'Игрок победил' : 'Орион победил'
			});
		}
...
export const makeRouletteAction = onCall(
	...
	async (req) => {
		...
		pub.events =
			events.length > 0
				? events
				: [{ type: 'game_over', actor: 'orion', log: 'No events' } as GameEvent];
